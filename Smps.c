...
static inline void smps_write_ym_note_len_direct(FILE *const f, uint32_t note_len, bool is_dac);
static void smps_write_ym_note_len(FILE *const f, const int32_t smp, bool is_dac);
...
static inline void smps_write_sn_note_len_direct(FILE *const f, int32_t note_len);
...
static void smps_fms_to_mod(struct SmpsModulationState *const mod_state, int fms);
...
static void smps_write_ym_freq(FILE *const f, const struct Ym2612State *const s)
{
	...
	if (s->freq != NOCHANGE)
	{
		...
		if (note_on)
		{
			...
			ARRAY_2D_UNSORTED_FIND_NEAREST(int16_t, logger->note_table, cd.fm.freq, oct_off, note_index);
...
static void smps_write_ym_note_len(FILE *const f, const int32_t smp, bool is_dac)
{
	...
	write_len_needed = cd.note_len != smp || ((cd.note == cd.note_prev) && cd.key_noattack);
	...
	const struct Sn76489NoiseState *const ns)
{
	...
	if (ns)
	{
		...
		if (ns->ch4_mode != NOCHANGE || ns->noise_type != NOCHANGE)
		{
			if (full_note_range_mode)
				psg_form_val = ns->noise_type == SN_NOISE_PERIODIC ? 0xE3 : 0xE7;
			else
				psg_form_val += 
	...
	if (tone != NOCHANGE)
	{
		...
		if (full_note_range_mode)
		{
			ARRAY_2D_SORTED_FIND_NEAREST(int16_t, sn76489_tone_table, tone, oct, note_index);
...
static void smps_write_ym_chls(FILE *const f)
{
	...
	for (int chl = 0; chl < YM_CHLS; ++chl)
	{
		...
		*ch_state_count = logger->ym2612_state_count[chl];
		...
		smps_header.fm[chl].chl_ptr = _byteswap_ushort(*chl_ptr);
		...
		if (*chl_ptr != *blank_chl_ptr)
		{
			if (loop)
			{
				...
				*loop_ptr = _byteswap_ushort(*loop_ptr);
...
static inline void smps_write_sn_note_len(FILE *const f, int32_t smp)
{
	...
	int32_t sep_lens[2] =
	{
		(loop && !(*loop_setuped) && loop_frame_overlapped ? (*frame_count - *loop_frames) : 0),
		smp - sep_lens[0]
	};
...
static void smps_write_sn_chls(FILE *const f)
{
	...
	for (int chl = 0; chl < SN_CHLS - 1; ++chl)
	{
		...
		*ch_state_count = logger->sn76489_state_count[chl];
		...
		smps_header.psg[chl].chl_ptr = _byteswap_ushort(*chl_ptr);
...
static void smps_write_sn_noise(FILE *const f)
{
	...
	*ch_state_count = logger->sn76489_noise_state_count;
	...
	smps_header.psg[2].chl_ptr = _byteswap_ushort(*chl_ptr);
	...
	for (uint32_t i = 0; i < *ch_state_count; ++i)
	{
		...
		*frame_count += s->frames;
		...
		smps_write_sn_note_len(f, s->frames, &(loop_state->frames));
	}
...
static void smps_write_ym_voices(FILE *const f)
{
	...
	if (ym_enabled)
	{
		...
		smps_header.voice_ptr = _byteswap_ushort(smps_header.voice_ptr);
...
void smps_write_dac(FILE *const f)
{
	...
	*ch_state_count = logger->ym2612_state_count[chl];
	...
	smps_header.dac.ptr = _byteswap_ushort(*chl_ptr);
...
void smps_export_to_file(const char *const file_path)
{
	...
	*loop = parser->vgm->vgm_header->loop_samples != 0;
	...
	if (*loop)
		*loop_frames = (parser->vgm->vgm_header->total_samples -
			parser->vgm->vgm_header->loop_samples) / logger->samples_per_frame;
	...
	if (!(*dac_enabled))
		smps_header.dac.full_val = _byteswap_ulong(SMPS_NODAC);
